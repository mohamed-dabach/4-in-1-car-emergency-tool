          ...
          {isSuccess ? (
            ...
            <>
              <div className="mb-5 text-center">
                <p className="text-sm font-black text-brand-red">التوصيل مجاني · خلّص عند الاستلام</p>
                <h2 className="font-display mt-1 text-[26px] font-extrabold sm:text-3xl">🛒 استمارة الطلب</h2>
                <p className="mt-1 text-sm text-brand-ink/60">عمّر المعلومات وغادي نتاصلو بيك نأكدو الطلب</p>
              </div>
              <form onSubmit={handleSubmit} className="space-y-4">
                <OfferPicker selected={offer} onSelect={selectOffer} />
                <div className="flex items-center justify-between rounded-xl bg-brand-ink px-4 py-3 text-white"><span className="text-sm font-bold">المجموع مع التوصيل</span><span className="text-2xl font-black tabular-nums text-brand-yellow">{offer.price} {product.currency}</span></div>
                <div className="grid gap-4 sm:grid-cols-2">
                  <Field label="الاسم الكامل"><input name="name" type="text" required autoComplete="name" placeholder="مثلا: فاطمة الزهراء" className={inputClass} onFocus={() => trackFormStarted('name')} /></Field>
                  <Field label="رقم الهاتف" error={phoneError}><input ref={phoneRef} name="phone" type="tel" required dir="ltr" inputMode="tel" autoComplete="tel" placeholder="06 XX XX XX XX" aria-invalid={Boolean(phoneError)} className={`${inputClass} text-right ${phoneError ? 'border-red-600 ring-1 ring-red-600' : ''}`} onFocus={() => trackFormStarted('phone')} onBlur={(e) => validatePhone(e.currentTarget.value)} /></Field>
                  ...
                  <Field label="العنوان"><input name="address" type="text" required autoComplete="street-address" placeholder="الحي، الزنقة، رقم الدار" className={inputClass} /></Field>
                </div>
                <div className="rounded-2xl border-2 border-brand-yellow bg-brand-yellow/15 p-3.5">
                  <p className="mb-2 flex items-center gap-1.5 text-sm font-black text-brand-ink"><Award className="h-5 w-5 text-brand-red" /> 🎨 سمية الطفل (كنطبعوها على الشهادة مجاناً)</p>
                  <div className={`grid gap-3 ${offer.qty === 2 ? 'sm:grid-cols-2' : ''}`}>
                    <Field label={<span className="sr-only">سمية الطفل</span>}><input name="child1" type="text" required autoComplete="off" placeholder={offer.qty === 2 ? 'سمية الطفل الأول' : 'مثلا: أنس'} className={inputClass} /></Field>
                    {offer.qty === 2 ? <Field label={<span className="sr-only">سمية الطفل الثاني</span>}><input name="child2" type="text" required autoComplete="off" placeholder="سمية الطفل الثاني" className={inputClass} /></Field> : null}
                  </div>
                </div>
                <button type="submit" data-cta="" disabled={isSubmitting} className="flex min-h-14 w-full touch-manipulation items-center justify-center gap-2 rounded-2xl bg-brand-red px-5 text-xl font-black text-white shadow-[0_12px_28px_-12px_rgba(240,71,59,0.9)] transition hover:brightness-105 active:scale-[0.98] disabled:cursor-wait disabled:opacity-60 focus-visible:outline-3 focus-visible:outline-offset-3 focus-visible:outline-brand-cyan">{isSubmitting ? <><LoaderCircle className="h-6 w-6 animate-spin" /> كنسجلو الطلب...</> : <><ShoppingCart className="h-6 w-6" /> اضغط هنا لتأكيد طلبك</>}</button>
                <ul className="flex flex-wrap justify-center gap-x-4 gap-y-2 text-xs font-bold text-brand-ink/65"><li className="flex items-center gap-1"><Truck className="h-4 w-4" /> توصيل مجاني وسريع</li><li className="flex items-center gap-1"><PackageSearch className="h-4 w-4" /> عاين سلعتك عاد خلص</li><li className="flex items-center gap-1"><Wallet className="h-4 w-4" /> الدفع عند الاستلام</li></ul>
